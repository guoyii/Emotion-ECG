...
def val_epoch(net,loader,cost):
    ...
    labels, predicts = [],[]
    loss_meter = AvgMeter()
    for batch_idx, (sequence, label,_) in enumerate(loader):
        sequence = sequence.float().cuda() #GPU
        label = label.float().cuda()  #GPU
        # sequence = sequence.float() #CPU
...
def main(args):
    ...
    if args.is_fine_tuning:
        ...
        result_path = os.path.join(args.output_path, "pretain_result")
    ...
    #加载预训练数据或微调数据
    if args.is_fine_tuning:
        train_set = MySet(args.fine_tuning_txt_path, mode="train")
        val_set = MySet(args.fine_tuning_txt_path, mode="val")
        ...
        train_set = MySet(args.pretain_txt_path, mode="train")
        val_set = MySet(args.pretain_txt_path, mode="val")
        train_loader = DataLoader(train_set, batch_size=args.pretain_batch_size, shuffle=True)
        val_loader = DataLoader(val_set, batch_size=args.pretain_batch_size, shuffle=False)
    ...
    if args.is_fine_tuning:
        ...
        weights=torch.load(weights_path,map_location='cuda:%d'%(args.gpu_id))  #GPU
        # weights=torch.load(weights_path,map_location='cpu')  #GPU
    ...
    t0=time.time()
    if args.is_fine_tuning:
        epoch_num = args.fine_tuning_epoch
    else:
        epoch_num = args.pretain_epoch
    ...
    if args.is_fine_tuning:
        torch.save(state, os.path.join(result_path, "Network_fine_tuning.pth.gz"))
    else:
        torch.save(state, os.path.join(result_path, "Network_pretain.pth.gz"))
