...
class MySet(Dataset):
    def __init__(self, txt_path, sequence_path, mode="train"):
        self.mode = mode
        self.data_list = read_list(self.mode, txt_path, sequence_path)
...
def read_list(mode, txt_path, sequence_path):
    if mode == "train":
    ...
    for line in lines:
        ...
        sequence_info = {
            "sequence": os.path.join(sequence_path, sequence_name),
            "label": int(label)
        }
